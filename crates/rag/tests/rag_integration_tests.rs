...
use anyhow::Result;
use ai_agent_common::{CollectionTier, ConversationId};
...
async fn test_multi_source_retriever_with_web_crawler() -> Result<()> {
    ...
    // Create multi-source retriever with web crawler enabled
    ...
    while let Some(result) = stream.next().await {
        match result {
            Ok(fragment) => {
                ...
                count += 1;
                if count > 20 { // Limit to prevent infinite loops in tests
                    break;
                }
            }
            Err(e) => {
